...
const GAME_DEFS = {
  ...
  // so this is the one game that needs a real Steam login. Current builds ship a 64-bit
...
async function runInstall(id, server, config) {
  ...
  if (server.game === 'Terraria' && server.terrariaType === 'tmodloader') {
    ...
    // FiveM FXServer — direct downloads from Cfx.re (no Steam / account needed).
    await installFiveM(id, server.installDir, config);
    ...
    for (let attempt = 1; attempt <= MAX_STEAM_ATTEMPTS; attempt++) {
      ...
      exePath = findExe(server.installDir, def.startExe) || (def.altExe ? findExe(server.installDir, def.altExe) : '');
      if (exePath) break;
    }
...
function syncServerConfig(serverId, server, isInstall = false) {
  ...
  try {
    if (server.game === 'Palworld') {
      ...
      const lic  = String(server.licenseKey || '').replace(/"/g, '');
      try { fs.mkdirSync(path.dirname(cfgPath), { recursive: true }); } catch (e) {}
      if (fs.existsSync(cfgPath)) {
        let cfg = fs.readFileSync(cfgPath, 'utf8');
        const patch = (re, line) => { cfg = re.test(cfg) ? cfg.replace(re, line) : (cfg + '\n' + line); };
        patch(/^endpoint_add_tcp .*$/m, `endpoint_add_tcp "0.0.0.0:${fmPort}"`);
        ...
        fs.writeFileSync(cfgPath, [
          `# Generated by Omnex — edit freely.`,
          `endpoint_add_tcp "0.0.0.0:${fmPort}"`,
          `endpoint_add_udp "0.0.0.0:${fmPort}"`,
          `sv_maxclients ${maxc}`,
          `sv_hostname "${host}"`,
          ...
          ``,
          `# Admins (add your own Cfx/Steam identifiers)`,
          `add_ace group.admin command allow`,
          `add_principal identifier.fivem:1 group.admin # example`,
          ``,
          `# Get a FREE license key at https://keymaster.fivem.net`,
          `sv_licenseKey "${lic}"`,
          ``,
        ].join('\n'));
      }
      if (isInstall) log(serverId, 'success', `✔ server.cfg written (port ${fmPort}).`);
    }

    else if (server.game === 'Arma 3') {
      // Arma's server.cfg is its own C-like format (key = value;), not ini/json — so
      // Omnex owns a small set of top-level lines and patches them in place, leaving
      // the user's mission classes and hand-added settings untouched.
      const cfgPath = path.join(server.installDir, 'server.cfg');
      const host = String(gameName).replace(/"/g, "'");
      const maxc = String(server.maxPlayers || 32);
      const pw   = String(server.armaPassword || '').replace(/"/g, '');
      const apw  = String(server.armaAdminPassword || '').replace(/"/g, '');
      if (fs.existsSync(cfgPath)) {
        let cfg = fs.readFileSync(cfgPath, 'utf8');
        const patchLine = (re, line) => { cfg = re.test(cfg) ? cfg.replace(re, line) : (cfg.replace(/\s*$/, '') + '\n' + line + '\n'); };
        patchLine(/^[ \t]*hostname[ \t]*=.*$/m,   `hostname = "${host}";`);
        patchLine(/^[ \t]*maxPlayers[ \t]*=.*$/m, `maxPlayers = ${maxc};`);
        // Only touch the passwords when the user actually set one in Omnex, so an
        // imported server's existing passwords are never silently blanked out.
        if (pw)  patchLine(/^[ \t]*password[ \t]*=.*$/m,      `password = "${pw}";`);
        if (apw) patchLine(/^[ \t]*passwordAdmin[ \t]*=.*$/m, `passwordAdmin = "${apw}";`);
        fs.writeFileSync(cfgPath, cfg);
      } else {
        fs.writeFileSync(cfgPath, [
          `// Generated by Omnex — edit freely.`,
          ...
          `kickDuplicate = 1;`,
          ``,
          `// 2 = only signed mods may connect (safe default). Lower it only if you know why.`,
          `verifySignatures = 2;`,
          `BattlEye = 1;`,
          ``,
          `motd[] = {"Managed by Omnex"};`,
          `motdInterval = 30;`,
          ``,
          ...
          `vonCodecQuality = 10;`,
          ``,
          `// Add missions here to rotate them automatically, e.g.`,
          `//   class Missions {`,
          `//     class MyMission { template = "MyMission.Altis"; difficulty = "Regular"; };`,
          `//   };`,
          `class Missions {};`,
          ``,
        ].join('\n'));
      }
      // -profiles=profiles points here; Arma writes its .rpt log and persistent
...
// FiveM (GTA V): download the recommended FXServer artifacts + the default server
// resources (cfx-server-data), and generate a server.cfg. No Steam / account needed
// for the download; the server needs a free Cfx.re license key to actually start.
async function installFiveM(serverId, installDir, config) {
  // 1. Resolve the recommended Windows FXServer build.
  log(serverId, 'info', 'Finding the recommended FiveM server build...');
  let artifactUrl = '';
  try {
    const v = await fetchJSON('https://changelogs-live.fivem.net/api/changelog/versions/win32/server');
    artifactUrl = (v && (v.recommended_download || v.latest_download)) || '';
  } catch (e) {}
  if (!artifactUrl) throw new Error('Could not reach the FiveM artifacts server.');

  // 2. Download + extract FXServer into installDir/server.
  const serverDir = path.join(installDir, 'server');
  fs.mkdirSync(serverDir, { recursive: true });
  const artZip = path.join(installDir, 'fxserver.zip');
  log(serverId, 'info', 'Downloading FXServer (FiveM) artifacts...');
  await downloadFile(artifactUrl, artZip, pct => {
    const f = Math.floor(pct / 5);
    emit('console-progress', { serverId, text: `⬇  FXServer  [${'█'.repeat(f)}${'░'.repeat(20 - f)}] ${pct}%` });
  });
  log(serverId, 'info', 'Extracting FXServer...');
  await extractZip(artZip, serverDir);
  try { fs.rmSync(artZip, { force: true }); } catch (e) {}
  const fx = findExe(serverDir, 'FXServer.exe');
  if (!fx) throw new Error('FXServer.exe not found in the downloaded artifacts.');

  // 3. Download the default server resources (cfx-server-data) into installDir/server-data.
  const dataDir = path.join(installDir, 'server-data');
  fs.mkdirSync(dataDir, { recursive: true });
  const dataZip = path.join(installDir, 'server-data.zip');
  const dataTmp = path.join(installDir, '_data_extract');
  try {
    log(serverId, 'info', 'Downloading default server resources...');
    await downloadFile('https://codeload.github.com/citizenfx/cfx-server-data/zip/refs/heads/master', dataZip, () => {});
    try { fs.rmSync(dataTmp, { recursive: true, force: true }); } catch (e) {}
    await extractZip(dataZip, dataTmp);
    // Hoist the archive's inner cfx-server-data-master/ contents up into server-data.
    const inner = fs.readdirSync(dataTmp).map(n => path.join(dataTmp, n)).find(p => { try { return fs.statSync(p).isDirectory(); } catch (e) { return false; } }) || dataTmp;
    for (const entry of fs.readdirSync(inner)) fs.cpSync(path.join(inner, entry), path.join(dataDir, entry), { recursive: true });
  } catch (e) {
    log(serverId, 'warn', `Could not fetch default resources (${e.message}). The server folder is still usable.`);
  } finally {
    try { fs.rmSync(dataZip, { force: true }); } catch (e) {}
    try { fs.rmSync(dataTmp, { recursive: true, force: true }); } catch (e) {}
  }

  server_setExecForFiveM(serverId, fx);
  syncServerConfig(serverId, appData.servers.find(s => s.id === serverId), true); // writes server.cfg
  log(serverId, 'success', '✔ FiveM (FXServer) installed. Add your free Cfx.re license key in Config, then Start.');
  log(serverId, 'warn', 'Get a license key at https://keymaster.fivem.net (free) — the server won\'t start without it.');
}
function server_setExecForFiveM(serverId, fxPath) {
...
// FiveM: fields live on the server object and are written into server.cfg on start.
GAME_CONFIG_DEFS['FiveM'] = {
  file: 'server-data/server.cfg',
  props: [
    { group: '🚗 FiveM Server', props: [
      { key: 'licenseKey', label: 'Cfx.re License Key', type: 'serverText', secret: true, basic: true,
        desc: "Required. Get a free key at keymaster.fivem.net — the server won't start without it." },
      { key: 'name',       label: 'Server Name (hostname)', type: 'serverText', basic: true },
      { key: 'maxPlayers', label: 'Max Players', type: 'serverNumber', placeholder: '32' },
      { key: 'port',       label: 'Port', type: 'serverNumber', placeholder: '30120' },
    ]},
  ],
};

// ── Arma 3 server.cfg (key = value;) ─────────────────────────────────────────
// Arma's config is its own C-like format, so it gets a dedicated parse/serialize
// pass rather than the generic ini/cfg one. Only TOP-LEVEL keys are read or
// rewritten — anything nested inside a `class Missions { ... }` block is left
// exactly as the user wrote it.
function armaCfgDepth(line, depth) {
  for (const ch of line) { if (ch === '{') depth++; else if (ch === '}') depth--; }
  return depth;
}
function parseArmaCfg(raw) {
  const props = {};
  let depth = 0;
  for (const rawLine of String(raw).split(/\r?\n/)) {
    ...
    if (startDepth !== 0 || line.startsWith('class')) continue;
    // Bare identifiers only, so array keys (motd[]) are skipped rather than mangled.
...
function patchArmaCfg(raw, props, def) {
  const strings = new Set(def?.strings || []);
  const fmt = (k, v) => `${k} = ${strings.has(k) ? '"' + String(v).replace(/"/g, "'") + '"' : String(v)};`;
  const remaining = new Map(Object.entries(props || {}));
  let depth = 0;
  const out = String(raw).split(/\r?\n/).map(rawLine => {
    const line = rawLine.trim();
    if (!line || line.startsWith('//') || line.startsWith('#')) return rawLine;
    const startDepth = depth;
    depth = armaCfgDepth(line, depth);
    if (startDepth !== 0 || line.startsWith('class')) return rawLine;
    const m = line.match(/^([A-Za-z_][A-Za-z0-9_]*)\s*=/);
    if (!m || !remaining.has(m[1])) return rawLine;
    const val = remaining.get(m[1]);
    remaining.delete(m[1]);
    return rawLine.match(/^\s*/)[0] + fmt(m[1], val);
  });
  // Anything the file didn't already carry gets appended at the end.
  for (const [k, v] of remaining) out.push(fmt(k, v));
  return out.join('\n');
}

GAME_CONFIG_DEFS['Arma 3'] = {
  file: 'server.cfg',
  type: 'armacfg',
  // Keys Arma expects as quoted strings; everything else is written bare.
  strings: ['hostname', 'password', 'passwordAdmin', 'serverCommandPassword', 'logFile', 'timeStampFormat'],
  props: [
    { group: '🌍 Server', props: [
      { key: 'hostname',              label: 'Server Name',      type: 'text',   basic: true },
      { key: 'password',              label: 'Join Password',    type: 'text',   basic: true, desc: 'Leave empty for a public server.' },
      { key: 'passwordAdmin',         label: 'Admin Password',   type: 'text',   desc: 'Used in-game with #login to take admin.' },
      { key: 'serverCommandPassword', label: 'Command Password', type: 'text',   desc: 'Lets a voted-in admin run server commands.' },
      { key: 'maxPlayers',            label: 'Max Players',      type: 'number', basic: true },
    ]},
    { group: '🛡 Security', props: [
      { key: 'verifySignatures', label: 'Verify Signatures',   type: 'select', options: ['0','1','2'],
        desc: '2 = only signed mods may connect (recommended). 0 = no checks.' },
      { key: 'BattlEye',         label: 'BattlEye Anti-Cheat', type: 'bool01' },
      { key: 'kickDuplicate',    label: 'Kick Duplicate IDs',  type: 'bool01' },
      { key: 'persistent',       label: 'Persistent Mission',  type: 'bool01',
        desc: 'Keeps the mission running after the last player leaves.' },
    ]},
    { group: '🗳 Voting', props: [
      { key: 'voteMissionPlayers', label: 'Players Before Voting', type: 'number' },
      { key: 'voteThreshold',      label: 'Vote Threshold',        type: 'number',
        desc: 'Share of players needed to pass a vote (0.33 = one third).' },
    ]},
    { group: '🎙 Voice', props: [
      { key: 'disableVoN',      label: 'Disable Voice Chat', type: 'bool01' },
      { key: 'vonCodecQuality', label: 'Voice Quality',      type: 'number', desc: '1–30; higher uses more bandwidth.' },
    ]},
    { group: '📝 Logging', props: [
      { key: 'logFile',         label: 'Log File',          type: 'text' },
      { key: 'timeStampFormat', label: 'Timestamps',        type: 'select', options: ['none','short','full'] },
      { key: 'motdInterval',    label: 'MOTD Interval (s)', type: 'number' },
    ]},
  ],
};
...
ipcMain.handle('read-server-config', (e, id) => {
  ...
  if (def) {
    ...
    // Arma 3: write the seed server.cfg if there isn't one yet (e.g. an imported
    // server that never had one), so the editor is usable immediately.
    if (!configPath && server.game === 'Arma 3') {
      try {
        syncServerConfig(server.id, server, /* isInstall */ false);
        const expected = path.join(server.installDir, 'server.cfg');
        if (fs.existsSync(expected)) configPath = expected;
      } catch (e) {}
    }
...
async function createBackup(serverId, label = 'Manual backup', trigger = 'manual') {
  ...
  const filesToBackup   = ['server.properties', 'ops.json', 'whitelist.json', 'banned-players.json', 'banned-ips.json',
                           'server.cfg', 'basic.cfg'];  // Arma 3 configs (copied only when present)
...
ipcMain.handle('import-server', async (e, config) => {
  ...
  try {
    ...
    let execPath = '';
    if (def?.startExe) execPath = findExe(installDir, def.startExe) || (def.altExe ? findExe(installDir, def.altExe) : '') || '';
...
ipcMain.handle('detect-server', async (e, dir) => {
  ...
  if (!game) {
    for (const [name, def] of Object.entries(GAME_DEFS)) {
      ...
      if (findExe(dir, def.startExe) || (def.altExe && findExe(dir, def.altExe))) { game = name; break; }
    }
  }
...
function getGameLogPath(server) {
  ...
  switch (server.game) {
    ...
    case 'Enshrouded':
      return findLatestLog(path.join(server.installDir, 'logs'), '.log');
    case 'Arma 3':
      // -profiles=profiles puts the .rpt (Arma's real console log) here.
      return findLatestLog(path.join(server.installDir, 'profiles'), '.rpt');
    default:
      return null;
  }
}

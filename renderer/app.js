...
// ── DEBUG: Always-on input state indicator ───────────────────────────────────
let __lastClickedEl = null;
document.addEventListener('mousedown', e => { __lastClickedEl = e.target; }, true);

function __runInputDiagnostic(el) {
  ...
  return {
    ...
    opacity: cs.opacity,
    elementAtPoint: elemAtPoint === el ? '✓ self' : (elemAtPoint?.tagName + '#' + (elemAtPoint?.id || '?') + '.' + (elemAtPoint?.className || ''))
  };
}
...
document.addEventListener('keydown', e => {
  if (e.key !== 'F2') return;
  e.preventDefault();
  const el = __lastClickedEl || document.activeElement;
  const diag = __runInputDiagnostic(el);
  if (!diag) {
    alert('Click on the broken input field first, then press F2.');
    return;
  }
  ...
  alert(`Input Diagnostic:

Tag: ${diag.tag}
ID: ${diag.id}
Class: ${diag.classList || '(none)'}
Disabled: ${diag.disabled}
ReadOnly: ${diag.readonly}

-webkit-app-region: ${diag.webkitAppRegion}
pointer-events: ${diag.pointerEvents}
user-select: ${diag.userSelect}

Element at click point: ${diag.elementAtPoint}

If "Element at click point" is not "self",
something is overlapping the input.

Force-fix applied. Try typing now.`);
});

// ── setupInputFix REMOVED - was causing focus loss on inputs ──
